...
public final class FeatureConverter {
  ...
  public Feature convert(String query, String context) {
    ...
    List<Long> inputIds = tokenizer.convertTokensToIds(tokens);
    List<Long> inputMask = new ArrayList<>(Collections.nCopies(inputIds.size(), 1L));
...
//    return new Feature(inputIds, inputMask, segmentIds, origTokens, tokenToOrigMap);
    return new Feature(inputIds, inputMask, segmentIds,tokens, tokenToOrigIndex);
  }
}
